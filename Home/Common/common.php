...
function browinfo(){
	if(strpos($_SERVER["HTTP_USER_AGENT"],"MSIE")){
		if(strpos($_SERVER["HTTP_USER_AGENT"],"MSIE 10.0"))
			$visitor_browser = "IE 10.0";
		else if(strpos($_SERVER["HTTP_USER_AGENT"],"MSIE 9.0"))
			$visitor_browser = "IE 9.0";
		else if(strpos($_SERVER["HTTP_USER_AGENT"],"MSIE 8.0"))
			$visitor_browser = "IE 8.0";
		else if(strpos($_SERVER["HTTP_USER_AGENT"],"MSIE 7.0"))
			$visitor_browser = "IE 7.0";
		else $visitor_browser = "MSIE";
	}
	else if(strpos($_SERVER["HTTP_USER_AGENT"],"Trident/7.0"))
			$visitor_browser = "IE 11.0";
	...
	else $visitor_browser = 'others';
	return $visitor_browser;
}

function getCids($cid)
{
	$rs = M("Category") -> where("ischecked=1 and cid=".$cid) -> order("displayorder desc,id asc") -> select();
	$arr = array();
	foreach($rs as $k => $v)
	{
		$arr[] = $v['id'];
...
function phpmail( $To, $Subject, $Body )
{
	...
	$subSet = M("Subsetting") -> find();
	
	$mail = new PHPMailer(); //建立邮件发送类
	$mail->IsSMTP(); // 使用SMTP方式发送
	$mail->Host = $subSet['smtp_server']; // 您的企业邮局域名
	$mail->SMTPAuth = true; // 启用SMTP验证功能
	$mail->Username = $subSet['smtp_username']; // 邮局用户名(请填写完整的email地址)
	$mail->Password = $subSet['smtp_password']; // 邮局密码
	$mail->Port=$subSet['smtp_port'];
	$mail->From = $subSet['smtp_send_mail']; //邮件发送者email地址
	$mail->FromName = $subSet['smtp_send_name'];
	$mail->AddAddress($To, "Client");//收件人地址，可以替换成任何想要接收邮件的email信箱,格式是AddAddress("收件人email","收件人姓名")
	//$mail->AddReplyTo("", "");
