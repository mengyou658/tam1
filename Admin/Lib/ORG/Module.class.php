			...
			for($j=0; $j<$fNum; $j++)
			{
				...
				switch($DRs[$j]["formType"])
				{
					case "select":
						...
						$DRs[$j]["FieldNameStr"] .= '<option value="">Please Select'.$DRs[$j]["Name"].'</option>'.chr(10).chr(13);
					...
					break;
					case "upfile":
					$DRs[$j]["FieldNameStr"] = '<div class="upload_btn-group"><input name="upfile_'.$DRs[$j]["fieldName"].'" type="file" id="upfile_'.$DRs[$j]["fieldName"].'" /></div><div class="bigfileQueue" id="fileQueue_'.$DRs[$j]["fieldName"].'"></div><div class="showupfile" name="info_'.$DRs[$j]["fieldName"].'" id="info_'.$DRs[$j]["fieldName"].'">Uploaded File：<input type="text" class="txt" name="'.$DRs[$j]["fieldName"].'" id="'.$DRs[$j]["fieldName"].'" value="'.$DRs[$j]["fieldValue"].'"/>';
					if($DRs[$j]["fieldValue"])
					{
						$DRs[$j]["FieldNameStr"] .= "<span id=\"uped_".$DRs[$j]["fieldValue"]."\"><input class=\"btn_b\" type=\"button\" value=\" View Uploaded \" onclick=\"window.open('__APP__/Public/download?fid=".$DRs[$j]["fieldValue"]."')\"><input class=\"btn_b\" type=\"button\" value=\" Delete \" onclick=\"clearHtml('".$DRs[$j]["fieldName"]."');\"></span>";
					}
					...
					if($DRs[$j]["fieldValue"])
					{
						...
						for ($i = 0; $i < $countImg; $i++) {
							$sn = "";
							$sn = time() . $i;
							$ImgHtml .= '<dl id=\'' . $sn . '\'><dt><input type=\'hidden\' name=\''.$DRs[$j]["fieldName"].'[]\' value=\'' . $img[$i]['id'] . '\'><a href=\'' . $img[$i]['attachment'] . '\' rel=\'rel_'.$DRs[$j]["fieldName"].'\' class=\'lightbox\'><img src=\'' . $img[$i]['attachment'] . '\' class=\'sImg\'></a></dt><dd><a href=\'javascript:MoveHtml(' . $sn . ')\'>Delete <img src=\'__PUBLIC__/admin/images/del.gif\' align=\'absbottom\'></a><br /><input type=\'text\' name=\'displayorder[]\' value=\'' . $img[$i]['displayorder'] . '\' style="width:80px;text-align:center;" ></dd></dl>';
						}
					...
					$DRs[$j]["FieldNameStr"] .= "<script type=\"text/javascript\">
									$(document).ready(function() {
										$(\"#up_".$DRs[$j]["fieldName"]."\").uploadify({
											...
											'cancelImg'      : '__PUBLIC__/admin/js/JqueryUpload/cancel.png',
											'folder'         : \"__ROOT__/Attachments/upload/".date('Ymd')."/\",
											'buttonImg' 	 : '".__PUBLIC__."/admin/images/browser1.png',
					...
					case "file":
						...
						if($DRs[$j]["fieldValue"] != "") 
						{
							$DRs[$j]["FieldNameStr"] .= '<div id="'.$DRs[$j]["fieldName"].'_Html"><input type="hidden" name="old_'.$DRs[$j]["fieldName"].'" value="'.$DRs[$j]["fieldValue"].'" /><a href="'.$DRs[$j]["fieldValue"].'" class="lightbox"><img src="'.$DRs[$j]["fieldValue"].'" class="sImg"></a>&nbsp;&nbsp;<input type="button" class="btn_b" name="dels" value=" Delete Image " onclick="MoveHtml(\''.$DRs[$j]["fieldName"].'_Html\');"></div>';
						}
					break;					
					case "date":
