...
class DatabaseModel extends Model {
	...
	public function chkTable($table) {
	...
	public function createModule($cTable) {
		$chk = $this -> chkTable($cTable);
		if($chk) return FALSE;
		
		M('')->query('create table if not exists `'.$cTable.'` ( `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,   `ischecked` tinyint(1) NOT NULL DEFAULT "1",`isgood` tinyint(1) NOT NULL DEFAULT "0",`cid` int(8) NOT NULL DEFAULT "0", `postdate` varchar(18) collate utf8_unicode_ci NOT NULL,  `editdate` varchar(18) collate utf8_unicode_ci NOT NULL,`hits` int(8) NOT NULL DEFAULT "0", `lang` varchar(18),`displayorder` int(8) NOT NULL DEFAULT "0" collate utf8_unicode_ci NOT NULL, UNIQUE KEY `id` (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
		return TRUE;
	}
	
	public function renameTable($from , $to) {
		M() -> query("RENAME TABLE  ".$from."  TO  ".$to." ;");
	}
}