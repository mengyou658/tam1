...
<!--<div class="RootName">分类信息</div>
        <div class="line1px"></div> 
<table> 
    <tr>
        <td align="right" width="20%"><span class="need">*</span> 所属分类：</td>
        <td align="left">
...
<!--<?php if(($cid) != ""): ?>-->
    <?php if(is_array($groupRs)): $i = 0; $__LIST__ = $groupRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$groupRs): $mod = ($i % 2 );++$i;?><div class="RootName"><a href="javascript:;" onclick="history.go(-1);">Back</a></div>
        <div class="line1px"></div>
     ...
     <?php if(($vo["attachment"]) != ""): ?><table width="100%"  border="0">
     <tr>
     	<td width="20%" align="right">File Upload:</td>
        <td width="80%" align="left"><a href="/admin.php/Public/download?fid=<?php echo ($vo["attachment"]); ?>" target="_blank">Download File</a></td>
     </tr>
     </table><?php endif; ?>
    <div class="submit_s">
        <!--<input type="hidden" name="id" id="id" value="<?php echo ($vo["id"]); ?>" />
        <input type="submit" class="btn_b" name="submit_btn" value="确定" />
        <input type="reset" name="reset_btn" class="btn_w" value="重置" />-->
    </div>
<!--   <?php else: ?>
    <div class="msg">
     	请先选择分类....
