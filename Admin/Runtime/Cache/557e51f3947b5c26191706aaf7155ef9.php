...
<div class="search" id="serach">
    <form method="get" name="searchform" id="searchform" action="__URL__">
        ...
        <option value="name" <?php if(($_GET['field']) == "name"): ?>selected<?php endif; ?>>Name</option>
        <option value="email" <?php if(($_GET['field']) == "email"): ?>selected<?php endif; ?>>Email</option>
        <option value="content" <?php if(($_GET['field']) == "content"): ?>selected<?php endif; ?>>Message</option>
        </select>
        &nbsp;  
...
<form name="form1" method="post" action="__URL__/submit">
    <table border="0" cellspacing="0" cellpadding="0">
      ...
      <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($k % 2 );++$k;?><tr overstyle='on' id="app<?php echo ($list["id"]); ?>">
            ...
            <td nowrap="nowrap"><a href="__URL__/edit/id/<?php echo ($list["id"]); ?>"><?php if($list['isgood'] == 0): ?><span class="need">Unread</span><?php else: ?>Read<?php endif; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript:void(0);" onclick="delItem('<?php echo ($list["id"]); ?>','__URL__/doDelete');">Delete</a></eq>
            </td>
